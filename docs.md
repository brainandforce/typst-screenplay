...
Please make sure to download and install either the font `Courier`, `Courier New`, `Courier Prime`, or some other Courier clone, because the builtin mono-spaced fonts on typst are horrific.

`Courier`, `Courier New`, and `Courier Prime` are natively supported, but you can use another font by inserting a `set` rule like so:
